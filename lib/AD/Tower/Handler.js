...
AD.TowerHandler.prototype.update = function(fps) {
  ...
  var tower = null, activeEnemies = null, bulletVelocity = 10;
  for (var i = 0; i < this.towers.length; i++) {
    ...
    if (activeEnemies.length > 0 && this.towers[i].bullets.length == 0) {
      for (var j = 0; j < activeEnemies.length; j++) {
        ...
        var newXD = tX - cosAlpha * (dET/1.5), newYD = tY - sinAlpha * (dET/1.5);

        // draw line to target for tests
        var gfx = new PIXI.Graphics();
        gfx.lineStyle(3, 0xFFFFFF);
        gfx.moveTo(tX, tY);
        gfx.lineTo(newXD, newYD);
        ad.game.display.container.addChild(gfx);

        this.towers[i].bullets.push("dummy");
        break; // just one bullet for tests
      }
    }
  }
}