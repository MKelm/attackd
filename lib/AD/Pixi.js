...
AD.Pixi.prototype.constructor = AD.Pixi;

AD.Pixi.prototype.autoResizeRenderer = function() {
...
AD.Pixi.prototype.animate = function(scope) {
  ...
  if (ad.game.run == true) {
    ad.game.update(ad.game);
  }
  scope.renderer.render(scope.stage);

  if (ad.userConfig.fps > -1) {
    global.setTimeout(function() { requestAnimFrame(scope.animate.curry(scope)); }, 1000 / ad.userConfig.fps );
  } else {
    requestAnimFrame(scope.animate.curry(scope));
  }
}

AD.Pixi.prototype.setLoader = function(activate) {
  if (activate === true) {
...
AD.Pixi.prototype.loadAssets = function(callback) {
  this.setLoader(true);
  var assetsToLoad = [
    "data/gfx/blank.png", "data/gfx/field_dirt.png", "data/gfx/field_grass.png",
    "data/gfx/fooshl.png", "data/gfx/tower_canon.png", "data/gfx/tower_gun.png",
    "data/gfx/tower_laser.png", "data/gfx/tower_quake.png"
  ];
  loader = new PIXI.AssetLoader(assetsToLoad);
  var scope = this;
  loader.onComplete = function() { scope.setLoader(false); callback.call(this); };
  loader.load();
}

AD.Pixi.prototype.setFpsCounter = function(keyEvent, scope) {
  ...
  if (typeof scope.fps == "undefined") {
    ...
    if (keyEvent === true && scope.fps[0] < 0) {
      ...
      ad.intervals["fpsCounter"] = setInterval(scope.setFpsCounter.curry(false, scope), 1000);
      keyEvent = false;
    ...
    if (keyEvent === true && scope.fps[0] > -1) {
      clearInterval(ad.intervals["fpsCounter"]);
      scope.fps[1].visible = false;
      scope.fps[0] = -1;
