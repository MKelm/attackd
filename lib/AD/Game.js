...
AD.Game.prototype.start = function() {
  ...
  ad.intervals.gameStep = setInterval(
    this.step.curry(this), 1000 / ((ad.userConfig.fps > -1) ? ad.userConfig.fps : 30)
  );
}
...
AD.Game.prototype.step = function(scope) {
  ...
  var tower = null, enemy = null;
  for (var i = 0; i < scope.towerHandler.towers.length; i++) {
    tower = scope.towerHandler.towers[i];
    for (var j = 0; j < scope.enemyHandler.enemies.length; j++) {
      enemy = scope.enemyHandler.enemies[j];

      var tX = tower.gfx.position.x + 50, tY = tower.gfx.position.y + 50, tR = 204;
      var eX = enemy.gfx.position.x, eY = enemy.gfx.position.y, eR = 10;
      if (Math.sqrt(Math.pow(tX - eX, 2) + Math.pow(tY - eY, 2)) < tR + eR) {
        console.log("enemy in tower range");
      }
    }
  }
  scope.towerHandler.moveBullets();
...
AD.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    ...
    case "map-field-click":
      var fieldCoor = event.content.mouse.target.parent.position;
