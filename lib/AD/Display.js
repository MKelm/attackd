...
AD.Display.prototype.drawDiagonalLines = function(targetGfx, width, height, lineSpace) {
...
AD.Display.prototype.drawFieldArea = function(map) {
  ...
  for (var fY = 0; fY < 10; fY++) {
    ...
    for (var fX = 0; fX < 10; fX++) {
      ...
      this.drawField(coorX, coorY, fieldType, fX, fY);
      coorX += 102;
    }
    coorY += 102;
  }
...
AD.Display.prototype.drawField = function(coorX, coorY, fieldType, fX, fY) {
  var fWidth = 102, fHeight = 102;
  var fieldGfx = new PIXI.DisplayObjectContainer();
  fieldGfx.position = { x: coorX, y: coorY };
  ...
  gfx.lineStyle(3, 0xFFFFFF)
  gfx.drawRect(0, 0, fWidth, fHeight);
  ...
  if (fieldType != 0) {
    ...
    // this.drawDiagonalLines(fieldGfx, fWidth, fHeight); too much performance loss
  }

  // click event sprite for field interaction (towers -> field)
  if (fieldType == 0) {
...
AD.Display.prototype.drawInventoryArea = function(inventory) {
  var invWidth = 256 / 1.5, invHeight = 1024;
  var invGfx = new PIXI.DisplayObjectContainer();
  invGfx.pivot = {x: 0.5, y: 0.5 };
  invGfx.position = { x: 640 - invWidth, y: -0.5 * invHeight };
  ...
  // draw diagonal background stripes
  // this.drawDiagonalLines(invGfx, invWidth, invHeight); too much performance loss
  ...
  var style = {font: Math.floor(38 * ad.pixi.screen.ratio) + "px " + "Arial", fill: "4E4E4E"};
...
AD.Display.prototype.drawTower = function (type, x, y, inventoryData, targetGfx) {
  ...
  if (typeof inventoryData != "undefined") {
    ...
    // draw tower range in map fields
    var radius = 204;
    var gfx = new PIXI.Graphics();
    gfx.position = { x: tWidth/2, y: tHeight/2 };
    gfx.lineStyle(3, 0xFFFFFF);
...
AD.Display.prototype.drawGunTower = function(tGfx) {
  ...
  gfx.drawRect(70, 25, 25, 10);

  gfx.drawRect(70, 45, 25, 10);

  gfx.drawRect(5, 25, 25, 10);

  gfx.drawRect(5, 45, 25, 10);
