...
AD.Tower.prototype.addBullet = function(enemy, fps) {
  ...
  if (this.bullets.length < this.maxBullets) {
    ...
    // todo: improve distance / speed calculation !!!
    var calculate = true, step = 1, enemyDistance = 0, bulletDistance = 0;
    do {
      enemyDistance = step * enemySpeed;
      bulletDistance = step * currentSpeed;
      switch (enemy.position.direction) {
        case 2: // down
          varEnemyY = enemyY + enemyDistance;
          break;
        case 4: // left
          varEnemyX = enemyX - enemyDistance;
          break;
        case 8: // up
          varEnemyY = enemyY - enemyDistance;
          break;
      ...
      var nextX = towerMidX - cosAlpha * bulletDistance, nextY = towerMidY - sinAlpha * bulletDistance;

      // detect bullet hit
      if (Math.sqrt(Math.pow(nextX - varEnemyX, 2) + Math.pow(nextY - varEnemyY, 2)) < 1 + 10) {
        bulletHit = true;
      }
