    ...
    if (this.bulletTiming.lastShoot == -1 || shootTimeDiff >= this.bulletTiming.delay) {
      ...
      switch (this.type) {
        ...
        case 2: // laser
          ...
          break;
        case 3: // quake
          bullet.gfx = this.game.display.drawQuakeWave(towerMidX, towerMidY, size);
          bullet.waveRadius = 1;
          break;
      }
    ...
    if (this.targetRange > this.bullets[i].distance.current) {
      ...
      if (this.type == 0 || this.type == 1) {
        ...
        this.bullets[i].gfx = this.game.display.drawLaserBeam(
          towerMidX, towerMidY,
          this.bullets[i].targetEnemy.gfx.position.x, this.bullets[i].targetEnemy.gfx.position.y
        );
        if (this.bullets[i].distance.current > this.bullets[i].distance.target) {
          // lase beam impulse to enemy hit
