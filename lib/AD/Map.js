...
AD.Map = function() {
  ...
  this.pointer = { start: 1, current: 1, end: 3 };
  this.path = [];
}

AD.Map.prototype.constructor = AD.Map;

AD.Map.prototype.load = function() {
  var map = ad.util.loadJSON('./lib/data/maps/' + this.pointer.current + '.json');
  this.fields = map.fields;
  this.startPosition = map.start;
  this.endPosition = map.end;
  if (this.pointer.current < this.pointer.end) {
    this.pointer.current++;
    return true; // further map available
  }
  return false; // no further map available
}
